...
//        System.out.println(ob.maxProfit2(prices, fee));
    ...
    public int maxProfit2(int[] prices, int fee) {
        ...
        for (int i = 1; i < prices.length; i++) {
            tmpbuy = Math.max(noAction + -prices[i], Math.max(-prices[i], sell == null ? 0 : sell) + -prices[i]);

            if (hold != null)
                tmpsell = Math.max(prices[i] + buy - fee, prices[i] + hold - fee);
            else
                tmpsell = prices[i] + buy - fee;
    ...
    // Recursive sol based on recursion
    public int maxProfit(int[] prices, int fee) {
        if (prices == null || prices.length == 0)
            return 0;

        Map<Action, List<Action>> nextActionLookup = new HashMap<>();
        nextActionLookup.put(Action.NoAction, Arrays.asList(Action.Buy, Action.NoAction));
        nextActionLookup.put(Action.Buy, Arrays.asList(Action.Sell, Action.Hold));
        nextActionLookup.put(Action.Sell, Arrays.asList(Action.Buy, Action.NoAction));
        nextActionLookup.put(Action.Hold, Arrays.asList(Action.Hold, Action.Sell));
        Map<String, Integer> cache = new HashMap<>();

        return Math.max(maxProfitUtil(prices, 1, Action.NoAction, nextActionLookup, fee, cache),
                maxProfitUtil(prices, 1, Action.Buy, nextActionLookup, fee, cache) + -prices[0]);
    }



    private int maxProfitUtil(int[] prices, int index, Action lastAction,
                              Map<Action, List<Action>> nextActionLookup, int fee,
                              Map<String, Integer> cache) {
        if (index >= prices.length) {
            return 0;
        }

        if (cache.containsKey(String.valueOf(index) + lastAction)) {
            return cache.get(String.valueOf(index) + lastAction);
        }

        int max = Integer.MIN_VALUE;
        for (Action currentAction : nextActionLookup.get(lastAction)) {

            if (currentAction == Action.NoAction)
                max = Math.max(max,
                        maxProfitUtil(prices, index + 1, Action.NoAction, nextActionLookup, fee, cache));

            if (currentAction == Action.Buy)
                max = Math.max(max,
                        maxProfitUtil(prices, index + 1, Action.Buy, nextActionLookup, fee, cache) + -prices[index]);

            if (currentAction == Action.Hold)
                max = Math.max(max,
                        maxProfitUtil(prices, index + 1, Action.Hold, nextActionLookup, fee, cache));

            if (currentAction == Action.Sell)
                max = Math.max(max,
                        maxProfitUtil(prices, index + 1, Action.Sell, nextActionLookup, fee, cache) + prices[index] - fee);
        }

        cache.put(String.valueOf(index) + lastAction, max);
        return max;
    }
